...
the primary-law databases (`constitution.db`, `statutes.db`, `rules.db`,
`admincode.db`) and the Attorney General and Judicial Ethics Advisory
Committee databases (`ag_opinions.db`, `jeac_opinions.db`) — are described
below.
...
- **Opinions of the North Dakota Supreme Court.** Judicial opinions of state
  ...
  dissents, and rehearing material when published with the opinion. Where a
  publication prints the court's own narrative statement of facts inside an
  editorial synopsis block, that narrative is the court's work and is
  retained; only the editorial wrapper around it is removed.
...
- **West editorial headnotes**, key-number content, synopses, and holding
  summaries. All of this is proprietary editorial work of Thomson Reuters and
  is filtered out from any source material.
...
The [Caselaw Access Project](https://lil.law.harvard.edu/projects/caselaw-access-project/) digitized the bound *North
Western Reporter* volumes and publishes the results as bulk data — per-case
JSON and page images — which this project uses as an independent witness for
the 1954–1996 range (page-exact against the printed volume) and as a source
of bound-volume text where no court-published copy survives. CAP's data is
released for reuse without restriction; the court-authored text it carries is
...
The court's own opinion text — as published in the *North Dakota
Reports* (volumes 1–79, 1890–1953) and the *North Western Reporter*
(N.W./N.W.2d) — supplies the primary text for the pre-1953 era and
supplements or cross-checks the 1953–1996 range where opinions are not
otherwise court-sourced. That published opinion text and the court-authored
syllabi are edicts of government not protected by copyright; only that
public-domain content is extracted and included in `opinions.db`. West's
proprietary editorial content — headnotes, key-number material, Synopsis
stubs, and "Procedural Posture" lines — is removed.
...
verification status against its best available source (court PDF,
bound reporter text, the court's archive HTML, or bound-volume scan) — 100.0% in
