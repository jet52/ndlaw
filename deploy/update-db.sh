...
for row in "${ROWS[@]}"; do
  ...
  # opinions: soft invariants re-check (hard-gated at release build).
  if [ "$name" = "opinions" ]; then
    if NDCOURTS_DB="$STAGE/$dbfile" "$VENV/bin/python" -m ndcourts_mcp.invariants 2>/dev/null \
         | grep -E 'Invariants:' | grep -q ' 0 regressed'; then
      say "    invariants clean"
    else
      say "    WARNING: invariants not clean on staged opinions.db (continuing; gated at build)"
    fi
    rm -f "$STAGE/$dbfile-wal" "$STAGE/$dbfile-shm"
  fi
