...
main() {
  ...
  # Full hostname (e.g. mcp.ndconst.org) for notification messages; falls
  # back to the short name if the box has no FQDN configured.
  local host
  ...
  sudo -u "$RUN_USER" git -C "$REPO" fetch --tags --prune --quiet origin \
    || { notify "ndlaw deploy FAILED ($tag): git fetch" \
         "git fetch failed on $host deploying $tag. No change made."; exit 1; }
  sudo -u "$RUN_USER" git -C "$REPO" -c advice.detachedHead=false checkout --quiet "$tag" \
    || { notify "ndlaw deploy FAILED ($tag): git checkout" \
         "git checkout $tag failed on $host. No change made."; exit 1; }
  sudo -u "$RUN_USER" bash -lc "cd '$REPO' && PATH=\"\$HOME/.local/bin:\$PATH\" uv pip install . >/dev/null 2>&1" \
  ...
  if release_has_db_assets; then
    ...
    if "$REPO/deploy/update-db.sh"; then
      ...
      notify "ndlaw deployed $tag" \
        "Server $host is now on $tag (opinions=$n). Code pinned to the tag and reinstalled; DB swapped and health-probed OK. Prior DB kept at $APP_HOME/opinions.db.bak."
    else
      notify "ndlaw deploy FAILED ($tag): update-db rolled back" \
        "update-db.sh failed/rolled back on $host deploying $tag; DB restored from .bak. NOTE: code is already at $tag, so code and DB may be mismatched — investigate before the next run. Logs: journalctl -u $SERVICE -n 50."
      exit 1
    fi
  else
    ...
    if health_probe; then
      echo "$tag" > "$MARKER"
      notify "ndlaw deployed $tag (code-only)" \
        "Server $host is now on $tag. Code reinstalled and the service restarted + health-probed OK; databases unchanged (no DB assets in this release)."
    else
      ...
      if [ "$deployed" != "none" ] && reinstall_code "$deployed"; then
        systemctl restart "$SERVICE"
        notify "ndlaw deploy FAILED ($tag): code rolled back to $deployed" \
          "Health probe failed after the code-only deploy of $tag on $host; rolled the code back to $deployed and restarted. Marker left at $deployed. Logs: journalctl -u $SERVICE -n 50."
      else
        notify "ndlaw deploy FAILED ($tag): ROLLBACK FAILED — service may be down" \
          "Health probe failed deploying $tag and the code rollback to '$deployed' also failed on $host. The service may be DOWN — investigate now. Logs: journalctl -u $SERVICE -n 50."
      fi
      exit 1
    fi
  fi
