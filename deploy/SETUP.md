...
> **You probably don't need this.** If you cloned the repo to *use* the data,
> run the local stdio server against the release databases (README → Quick
> start): every query is a local SQLite read, tens of milliseconds instead of
> a network round trip, and you can open the `.db` files directly for bulk
> analysis. This document is for the rare case of hosting your **own** copy
> over HTTP for other people or other machines.
...
`sudo self-update.sh`. For that to work non-interactively, the account you
SSH in as needs passwordless sudo for that one script — and nothing else.
Scope it to the script's absolute path, keep it in its own `sudoers.d`
fragment, and validate with `visudo -c` before you log out:

```bash
# <deploy-user> = the SSH login account; <install-dir> = where the repo lives
# on the server (this guide uses /srv/ndcourts/ndlaw-mcp).
echo '<deploy-user> ALL=(root) NOPASSWD: <install-dir>/deploy/self-update.sh' \
  | sudo tee /etc/sudoers.d/ndlaw-selfupdate
sudo chmod 440 /etc/sudoers.d/ndlaw-selfupdate
sudo visudo -c
```

If you don't want any passwordless sudo on the box, skip this: the daily
`ndlaw-update.timer` deploys new releases on its own, and `push-db.sh` only
saves you the wait.
