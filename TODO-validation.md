...
- [x] **Investigate the remaining 93 diff-audit <0.20 archive pairs**. **Done 2026-05-04** — characterized in `triage/archive-83-deep-triage-2026-05-04.md`. 10 of the 93 are westlaw+NW (separate item below); the 83 archive pairs split into five distinct failure modes:
  ...
  - [ ] **Type Y / contaminated citations (5)**: DB row has 2 neutral cites including the archive's actual neutral cite as a stray (e.g., opinion 13202 listed as both `2000 ND 138` AND `2000 ND 141` when only one is correct). The archive is for a DIFFERENT opinion (the supplemental publication / per-curiam follow-on) that needs its own row, mirror of the §1 6 supplemental publications. Fix: (a) insert new opinion row for each supplemental publication; (b) remove the stray neutral cite from the contaminated row; (c) detach the wrong archive linkage.
