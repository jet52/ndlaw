...
- [ ] Add a post-scrape step (invoked from `weekly_scrape.sh`) that runs `python -m ndcourts_mcp.ingest` (incremental) + `python -m ndcourts_mcp.merge_nd_metadata` + `cite_extract` + `quality_scan` against any new opinions. Log the delta. (As of 2026-05-04 this is still manual — six weeks of opinions piled up between releases without entering the DB.)
...
- [ ] Investigate the misplaced markdown file at `~/refs/nd/opin/markdown/79/673.md` — it's the only file under `markdown/` whose stem doesn't match `<year>ND<n>` and the only file that produces the lone "1 errors" line in `python -m ndcourts_mcp.ingest`. Likely belongs in `NW2d/79/` or `N.D./79/`; verify and move (or delete if a duplicate).
