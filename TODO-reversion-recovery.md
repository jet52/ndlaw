...
- OPEN (manual worklist = `triage/sig-drops-classified.csv`): [ ] **TRUE_TRUNCATION 92** — 74 multi-name MAJORITY panels needing POSITIONAL insert before an existing separate writing (NOT append; per-case PDF arbitration — some DB bodies substantively diverge from source, e.g. 16311 Sandstrom-vs-Crothers); 15 unparseable (spelled-out roles / embedded markers); 3 dateline/surrogate. [ ] CONTENT_LOSS 22 (substantive loss; overlaps 1997 contamination — verify vs PDF/bound vol). [ ] MARKER_GARBLED 7 + REVIEW 20. [ ] justice-name OCR garbles in panels (NEU-MANN/MAKING/YANDE WALLE) — separate cleanup. [ ] promote sigscan gap to an invariant (Phase 4).
