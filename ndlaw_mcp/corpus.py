...
def create_corpus_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.executescript("""
        ...
        -- External-content FTS5 whose content source is a VIEW joining each
        -- version to its provision: citation/heading live on `provisions`,
        -- text on `provision_versions`, and FTS5 can only read one table. A
        -- content source that lacks an indexed column (the pre-2026-08-29
        -- DDL named provision_versions directly) breaks 'rebuild',
        -- 'integrity-check', count(*), column reads, snippet()/highlight().
        -- Rows are still indexed explicitly (index_version_fts); no triggers.
        CREATE VIEW IF NOT EXISTS provisions_fts_content AS
            SELECT v.id AS id,
                   p.citation AS citation,
                   COALESCE(p.heading, '') AS heading,
                   v.text_content AS text_content
            FROM provision_versions v
            JOIN provisions p ON p.id = v.provision_id;
        CREATE VIRTUAL TABLE IF NOT EXISTS provisions_fts USING fts5(
            citation,
            heading,
            text_content,
            content='provisions_fts_content',
            content_rowid='id',
    ...
    migrate_provisions_fts(conn)
    conn.executescript(XREF_SCHEMA)
    conn.commit()


PROVISIONS_FTS_CONTENT = "provisions_fts_content"
_LEGACY_FTS_CONTENT = "content='provision_versions'"


def provisions_fts_is_legacy(conn: sqlite3.Connection) -> bool | None:
    """True if provisions_fts still names provision_versions as its content
    table (the misconfigured pre-2026-08-29 DDL); False if view-backed; None
    if the table does not exist."""
    row = conn.execute(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='provisions_fts'"
    ).fetchone()
    if row is None:
        return None
    return _LEGACY_FTS_CONTENT in (row[0] or "")


def migrate_provisions_fts(conn: sqlite3.Connection) -> bool:
    """Re-point a legacy provisions_fts at the content view and rebuild it.

    Idempotent: returns False (no-op) unless the stored DDL still says
    content='provision_versions'. Dropping an external-content FTS5 table
    touches only its shadow tables; 'rebuild' re-derives every posting from
    the view, so the index afterwards matches the live citation/heading/text
    exactly (rows indexed before a later heading or citation edit did not).
    Runs inside the caller's transaction; the caller commits."""
    if not provisions_fts_is_legacy(conn):
        return False
...
def provisions_fts_integrity_check(conn: sqlite3.Connection) -> str | None:
    """Verify provisions_fts against its content view. Returns None when
    clean, else the SQLite error text.

    Uses the `rank`=1 form: FTS5's default 'integrity-check' validates only the
    index structure and PASSES an index whose postings disagree with the
    content table; the `, 1` argument makes it read the content back and
    compare (verified 2026-08-29 against a deliberately wrong row). The
    statement is an INSERT, so SQLite demands a writable handle even though
    nothing is written — a mode=ro connection fails with 'attempt to write a
    readonly database'. A legacy (provision_versions-backed) index fails here
    with 'no such column: T.citation', which is the defect this reports."""
    try:
        conn.execute(
            "INSERT INTO provisions_fts(provisions_fts, rank) VALUES('integrity-check', 1)"
        )
...
def index_version_fts(
    ...
    FTS is content-backed by the provisions_fts_content view (version joined to
    its provision). Rows are indexed explicitly at insert time rather than via
    triggers, so the values passed here MUST equal what the view returns for
    this version once the caller's writes land — `heading` is stored as '' when
    NULL, exactly as the view COALESCEs it. provisions_fts_integrity_check
    catches any drift; 'rebuild' repairs it."""
    conn.execute(
