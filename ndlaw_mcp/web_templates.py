...
_STYLE = """
  ...
  table.tbl-prose { border-collapse: collapse; font-size: .88em;
                    line-height: 1.4; min-width: 100%; }
  table.tbl-prose caption { caption-side: top; text-align: left;
                            font-weight: bold; padding-bottom: .4em; }
  table.tbl-prose th, table.tbl-prose td { border: 1px solid #ccc;
            padding: .4em .55em; text-align: left; vertical-align: top; }
  table.tbl-prose th { background: #f4f4f2; font-weight: bold; }
...
def render_body(text: str, tables: dict | None = None) -> str:
    """Escape-then-transform ``text_content`` into fidelity-first HTML.

    ``tables`` maps a ``[Table N]`` index to its tables.db row (``layout`` and
    ``render_html``). A ``layout='prose'`` table renders as that real HTML
    table instead of the fixed-width ``<pre>``: its cells are sentences, so
    a monospace grid is both far too wide and silently shows the corpus's
    ``*…*`` emphasis markers as literal asterisks (JT 2026-08-24). Numeric
    tables — every table before 2026 ND 34 — keep the ``<pre>`` verbatim, and
    an install with no tables.db falls back to it for everything.
    """
    ...
    for i, raw in enumerate(lines):
        ...
        if tm:
            ...
            meta = (tables or {}).get(int(am.group(0))) if am else None
            if meta and meta["layout"] == "prose" and meta["render_html"]:
                out.append(f'<div class="tblwrap"{aid}>{meta["render_html"]}</div>')
                continue
