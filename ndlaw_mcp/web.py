...
from . import corpus, proofread, tables_corpus, web_templates
...
def _prov_page(request: Request, name: str, citation: str, canon: str,
    ...
    try:
        ...
        # A NULL start is the court's own gap: its version table dates the
        # page 01/01/0001 (45 rules as of 2026-08-30; CLERK-ERRATA.md).
        start = ver["effective_start"] or "date not published by the court"
        window = f"effective {start} – {ver['effective_end'] or 'current'}"
...
def _opinion_tables(conn, oid: int) -> dict:
    """tables.db rows for one opinion, keyed by the inline ``[Table N]`` index.

    Only the LAYOUT decides how render_body draws a table, so an install with
    no tables.db (or a table with no row) just falls back to the fixed-width
    block already inline in text_content."""
    if not tables_corpus.attach_tables(conn, read_only=True):
        return {}
    try:
        rows = conn.execute(
            "SELECT table_index, layout, render_html FROM tbl.opinion_tables "
            "WHERE opinion_id = ?", (oid,)).fetchall()
    except sqlite3.OperationalError:
        return {}                            # tables.db predates the layout column
    return {r["table_index"]: {"layout": r["layout"],
...
def _opinion_page(conn, request: Request, row) -> Response:
    ...
    if col("date_modified"):
        kind = (col("modified_kind") or "modified").capitalize()
        date_line += (f" · <b>{html.escape(kind)}</b> "
                      f"{html.escape(col('date_modified'))}")
