...
**Deliberately EXCLUDED from this batch (verified false positives or sensitive):** 12476 (column "…v. Schmidt" already correct; the frontmatter *short* `title` was the wrong field); 13804/15073/15796/15844 (modern estate/matter cases — 8-digit dockets MATCH, "Estate of X"/"Matter of Y" is a legitimate caption of the same case); 14223 (NDSU — same case, column caption is more complete); 12463 (Heustis/Heusis OCR variant, same attorney-reinstatement case). **12451 held for separate vetting:** an `L.D.C.` juvenile matter where the column is intentionally anonymized ("Ward County Social Service Board v. L.D.C.'s Mother") and the frontmatter caption ("Richter v. Clark") would DE-ANONYMIZE the mother — not changed. **Residual:** the scan only catches contamination where the wrong and right captions share no distinctive token; same-token crossings (e.g., two "State v. X" cases) would be missed.
